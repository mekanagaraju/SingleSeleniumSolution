...
public class SingleSeleniumSolution implements WebController
{
   ...
   public SingleSeleniumSolution(WebDriver webDriver)
   {
   ...
   @Override
   public WebElement findClickableElement(String locator, int timeout)
   {
      log.debug("debug: Looking for element: {}", locator);
   ...
   public void overrideIECertificateError()
   {
      ...
      if (isVisibleWithWait(CONTINUE, 10))
      {
         try
         {
    ...
    */
   @Override
   public boolean isVisibleWithWait(String locator, int timeout)
   {
      try
      {
      ...
      switch (browserType)
      {
         ...
         case INTERNET_EXPLORER:
            ...
            if (sysEnv == null)
            {
               ...
               if (sysEnv.isEmpty())
               {
                  log.info("Please set the webdriver.ie.driver in system environment variables and restart the PC OR copy all your webdrivers under 'C:/Selenium' location");
                  throw new RuntimeException("Failed to instantiate a WebDriver instance for " + browserName);
               }
               else
               {
   ...
   public String captureScreenshot(String fileName)
   {
      ...
      String location = "C:\\Reports\\Images\\" + fileName + ".png";
      try
      {
